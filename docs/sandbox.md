...
| `tools/` | Shared helpers. Agnostic mechanism only — restamping instances. Running a deck and the PDK's device parameters live in `sim/`, because the design's own simulations use them and design code may not import from here. |
